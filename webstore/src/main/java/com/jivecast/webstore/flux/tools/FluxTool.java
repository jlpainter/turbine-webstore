...
package com.jivecast.webstore.flux.tools;
...
public class FluxTool implements ApplicationTool, Recyclable {
	...
	private TurbineUserWrapper user = null;
	...
	public void init(Object runData) {
		...
		try {
			...
			if (user != null) {
				this.user = new TurbineUserWrapper(this.data.getUser());
			}
		} catch (Exception e) {
			log.error("Error initializing the flux tool: " + e.toString());
		}
	}
