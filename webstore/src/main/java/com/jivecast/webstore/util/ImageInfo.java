...
public class ImageInfo {
    ...
    /**
     *added by painter@JiveCast.com.com
     *
     * Match file extension to MIME_TYPE_STRINGS
     */
    private static final String[] MIME_EXTENSION =
        {"jpeg", "gif", "png", "bmp", "pcx",
         "iff", "ras", "bmp", "bmp", "bmp",
         "psd", "swf" };
    ...
    /**
     * If {@link #check()} was successful, returns a String with the
     * File name extension appropriate to the format of the file.
     * @return EXTENSION, e.g. <code>gif</code>
     */
    public String getFileExtension() {
        if (format >= 0 && format < MIME_EXTENSION.length) {
            if (format == FORMAT_JPEG && progressive)
            {
                return "pjpeg";
            }
            return MIME_EXTENSION[format];
        } else {
            return null;
        }
    }
